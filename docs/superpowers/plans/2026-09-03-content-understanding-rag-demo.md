...
Use deterministic `uniqueString(subscription().id, resourceGroup().id, environmentName)`. Create separate API, worker, cleanup, and ACR-pull identities. Accept a required `deploymentPrincipalId` for the local bootstrap principal. When nonempty `githubOwner`, `githubOwnerId`, `githubRepository`, and `githubRepositoryId` parameters are supplied, also create the GitHub deployment UAMI and a federated credential whose environment-scoped subject exactly matches the immutable ID-bound `sub` claim emitted by GitHub, with audience `api://AzureADTokenExchange` and issuer `https://token.actions.githubusercontent.com`.
